...
use dumle::{hook::UseState, html, CachedKeyedNode, Context, Either, Empty, Vnode};
        ...
        UseState::new(|state: &State, set_state| {
            html! {
                ...
                <div>
                {
                    ...
                    vec.into_iter().map(|i| CachedKeyedNode::of(i, html!{<div>{i.to_string()}</div>})).collect::<Vec<_>>()
                }
                </div>
                </div>
            }
